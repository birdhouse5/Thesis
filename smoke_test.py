...
class SmokeTest:
    ...
    def test_sp500_dataset_creation(self):
        """Test SP500 dataset creation with unified PortfolioDataset"""
        logger.info("Testing SP500 dataset creation...")
        
        from environments.data import PortfolioDataset
        
        # Create full SP500 dataset using new unified class
    ...
    def test_crypto_dataset_creation(self):
        """Test crypto dataset creation with unified PortfolioDataset"""
        logger.info("Testing crypto dataset creation...")
        
        from environments.data import PortfolioDataset
        
        # Create full crypto dataset using new unified class
    ...
    def test_dataset_loading_and_splits(self):
        """Test dataset loading and temporal splitting with unified class"""
        ...
        for split_name, dataset_split in splits.items():
            assert len(dataset_split) > 0, f"{split_name} split is empty"
            assert dataset_split.num_assets > 0, f"{split_name} has no assets"
            assert dataset_split.num_features > 0, f"{split_name} has no features"
