...
def experiment_to_training_config(exp: ExperimentConfig) -> TrainingConfig:
    ...
    return TrainingConfig(
        ...
        vae_lr=# 0.0003569748519315124, optuna 2             # 0.00004409096982106036, #optuna result1 - best
        policy_lr=0.00010793575227642601, # 0.00015584712043192588, optuna 2 # 0.00002329493575648219, #optuna result1 - best
        noise_factor=0.05, 
        random_policy=False,
        vae_beta=0.0709877778524465, # 0.0007435972826570025, # optuna result1 - best
        vae_update_freq=5,
        seq_len=200,
        episodes_per_task=3,
        batch_size=8192,
        vae_batch_size=1024,
        ppo_epochs=8,
        entropy_coef=0.006439257494565313, # 0.047315891962627706, optuna 2 # 0.0009083214087882104, # optuna result1 - best
        ...
        ppo_clip_ratio=0.13374380455551138, #0.1473333435286036, optuna 2 # 0.19086925122925438, # optuna result1 - best
        value_loss_coef=0.9981350917703767, # 0.5, ?
        max_grad_norm=0.5,
        gae_lambda=0.95,
        discount_factor=0.99,
        min_horizon=exp.min_horizon,
        max_horizon=exp.max_horizon,
        eta=0.026057381475720114, # optuna result1 - best
        rf_rate=0.02,
