...
# Removed: logger_config / CSVLogger imports
from environments.data_preparation import load_dataset  # if used elsewhere in your repo
...
# NEW: minimal, unified run logger
...
class Config:
    ...
    def __init__(self, config_file=None, trial=None):
        ...
        # Optional seed for reproducibility (used by RunLogger.seed_everything)
        self.seed = 42
...
def prepare_split_datasets(config):
    ...
    for split_name, dataset in datasets.items():
        ...
        # Reshape to (total_time, N, F) and (total_time, N)
...
def main():
    ...
    episodes_trained = 0
    best_val_reward = float('-inf')  # using avg_reward as "Sharpe" proxy
    vae = None
    policy = None
    ...
    try:
        ...
        while episodes_trained < config.max_episodes:
            ...
            for episode_in_task in range(config.episodes_per_task):
                ...
                # Log train episode metrics to unified CSV
                ...
                # Optional: per-step wealth curve (if available)
                if hasattr(train_env, 'capital_history') and len(getattr(train_env, 'capital_history', [])) > 0:
                    run.log_wealth_curve(episodes_trained, train_env.capital_history)

                task_rewards.append(episode_result.get('episode_reward', 0.0))
                episodes_trained += 1

                # Periodic lightweight eval logging
                if episodes_trained % config.log_interval == 0:
                    ...
                    # Treat avg_reward as Sharpe metric for monitoring
                    run.log_val(episodes_trained,
                                sharpe=val_results['avg_reward'],
                                reward=val_results['avg_reward'],
                                cum_wealth=val_results['avg_return'])

                    avg_task_reward = np.mean(task_rewards) if len(task_rewards) else 0.0
                    logger.info(f"Episode {episodes_trained:4d}: "
                                f"reward={episode_result.get('episode_reward', 0.0):8.4f}, "
                                f"task_avg={avg_task_reward:8.4f}")

                # Early stop on episode budget
            ...
            if episodes_trained % config.val_interval == 0:
                ...
                # Unified validation logging
                run.log_val(episodes_trained,
                            sharpe=val_results['avg_reward'],
                            reward=val_results['avg_reward'],
                            cum_wealth=val_results['avg_return'])

                # Save best model based on validation "Sharpe" (avg_reward)
                if val_results['avg_reward'] > best_val_reward:
                    best_val_reward = val_results['avg_reward']
                    save_path = Path(run.run_dir) / "checkpoints" / "best_model.pt"
                    torch.save({
            ...
            if episodes_trained % config.save_interval == 0:
                checkpoint_path = Path(run.run_dir) / "checkpoints" / f"checkpoint_ep{episodes_trained}.pt"
                torch.save({
                    'episode': episodes_trained,
                    'vae_state_dict': vae.state_dict(),
                    'policy_state_dict': policy.state_dict(),
        ...
        if vae is not None and policy is not None:
            ...
            best_model_path = Path(run.run_dir) / "checkpoints" / "best_model.pt"
            ...
            run.log_test(sharpe=test_results['avg_reward'],
                         reward=test_results['avg_reward'],
                         cum_wealth=test_results['avg_return'])

            # Save final results bundle
            final_path = Path(run.run_dir) / "checkpoints" / "final_results.pt"
            torch.save({
                'episode': episodes_trained,
                'vae_state_dict': vae.state_dict(),
                'policy_state_dict': policy.state_dict(),
        ...
        # Close unified logger (writes summary.csv)
        run.close()


if __name__ == "__main__":
    # Basic logging formatting
    logging.basicConfig(
        level=logging.INFO,
