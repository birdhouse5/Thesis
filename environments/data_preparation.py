...
def diagnose_data_quality(df: pd.DataFrame) -> None:
    """Diagnose data quality issues to understand rectangular structure problems"""
    logger.info("🔍 Diagnosing data quality issues...")
    
    # Check date coverage per ticker
    ticker_date_counts = df.groupby('ticker')['date'].nunique()
    logger.info(f"Date coverage per ticker:")
    logger.info(f"  Min dates: {ticker_date_counts.min()}")
    logger.info(f"  Max dates: {ticker_date_counts.max()}")
    logger.info(f"  Mean dates: {ticker_date_counts.mean():.1f}")
    
    # Show tickers with least coverage
    least_coverage = ticker_date_counts.nsmallest(5)
    logger.info(f"Tickers with least date coverage:")
    for ticker, count in least_coverage.items():
        ticker_data = df[df['ticker'] == ticker]
        start_date = ticker_data['date'].min().date()
        end_date = ticker_data['date'].max().date()
        logger.info(f"  {ticker}: {count} dates ({start_date} to {end_date})")
    
    # Check ticker coverage per date
    date_ticker_counts = df.groupby('date')['ticker'].nunique()
    irregular_dates = date_ticker_counts[date_ticker_counts != df['ticker'].nunique()]
    
    if len(irregular_dates) > 0:
        logger.info(f"\nFound {len(irregular_dates)} dates with incomplete ticker coverage:")
        
        # Sample some problematic dates
        sample_dates = irregular_dates.head(10)
        for date, ticker_count in sample_dates.items():
            missing_count = df['ticker'].nunique() - ticker_count
            present_tickers = set(df[df['date'] == date]['ticker'].unique())
            all_tickers = set(df['ticker'].unique())
            missing_tickers = all_tickers - present_tickers
            
            logger.info(f"  {date.date()}: {ticker_count}/{df['ticker'].nunique()} tickers "
                       f"(missing {missing_count}: {list(missing_tickers)[:3]}{'...' if len(missing_tickers) > 3 else ''})")
    ...
    logger.info(f"  Expected total rows: {df['date'].nunique() * df['ticker'].nunique()}")
    logger.info(f"  Actual total rows: {len(df)}")
    logger.info(f"  Missing rows: {df['date'].nunique() * df['ticker'].nunique() - len(df)}")
...
def create_dataset(output_path: str = "data/sp500_rl_ready_cleaned.parquet",
    ...
    Updated with better data quality handling.
    """
    ...
    try:
        ...
        logger.info("📥 Step 1/5: Downloading stock data...")
        raw_data = download_stock_data(tickers, start_date, end_date)
        
        # Step 1.5: Diagnose data quality (NEW)
        logger.info("🔍 Step 1.5/5: Diagnosing data quality...")
        diagnose_data_quality(raw_data)
        
        # Step 2: Add technical indicators
        logger.info("📊 Step 2/5: Adding technical indicators...")
        with_indicators = add_technical_indicators(raw_data)
        
        # Step 3: Normalize features
        logger.info("📏 Step 3/5: Normalizing features...")
        normalized = normalize_features(with_indicators)
        
        # Step 4: Clean data (improved)
        logger.info("🧹 Step 4/5: Cleaning data...")
        cleaned = clean_data(normalized)
        
        # Step 5: Final validation
        logger.info("✅ Step 5/5: Final validation...")
        expected_rows = cleaned['date'].nunique() * cleaned['ticker'].nunique()
        actual_rows = len(cleaned)
        
        if actual_rows != expected_rows:
            logger.warning(f"⚠️  Dataset not perfectly rectangular: {actual_rows} rows, expected {expected_rows}")
            # But continue anyway - the model can handle small imperfections
        else:
            logger.info(f"✅ Dataset is perfectly rectangular: {actual_rows:,} rows")
