...
class Trainer:
    ...
    def _update_vae(self, observations, actions, rewards):
        ...
        obs_seq = torch.stack([torch.tensor(obs, dtype=torch.float32) for obs in observations]).unsqueeze(0)  # (1, seq_len, 30, features)
        
        # Convert hierarchical actions to flat vectors
        action_seq = []
        for action in actions:
            # Flatten: [decisions, long_weights, short_weights]
            action_flat = torch.cat([
                action['decisions'].float(),
                action['long_weights'].float(), 
                action['short_weights'].float()
            ])
